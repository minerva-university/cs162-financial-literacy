...
const InternshipForm = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await postInternship(
        formData.title,
      ...
      setSubmitMessage('Internship submitted successfully!');

    } catch (error) {
      console.error('Error submitting internship:', error);
      setSubmitError('Failed to submit internship. Please try again.');
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-2">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-auto overflow-hidden">
        <div className="bg-blue-500 text-white p-4 flex items-center justify-between">
          <h2 className="text-xl font-bold">Add Internship</h2>
          <button 
            onClick={onClose}
